...
| UI-05 | Manufacturing availability | Availability review must remain readable above the manufacturing screen on desktop and mobile | Undefined dialog background variable made the wizard transparent and visually mixed it with the underlying page | Use the defined surface token and contain header, scrolling table, result and actions within the modal | styles | Low | Frontend suite, TypeScript and production build |
