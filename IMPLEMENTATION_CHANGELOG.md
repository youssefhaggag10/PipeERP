...
| RPT-2 | Browser output was a generic A4 table unlike the branded Desktop documents | **Fixed**: created one shared branded A4 renderer over an exact frontend-owned copy of the Desktop background for ordinary invoices, weight/card invoices, quotations, detailed/short customer statements and a statement summary page; added direct printing inside Sales only for posted invoices | `A4PrintDocuments.tsx`, `src/assets/a4_invoice_background.png`, `SalesPage.tsx`, `ReportsPage.tsx`, `styles.css`, reports schemas/service/tests | Browser-generated PDF is one exact A4 page; rendered visual inspection; backend `97 passed, 7 skipped`; frontend `23 passed`; production and frontend-isolated Docker build paths passed |
