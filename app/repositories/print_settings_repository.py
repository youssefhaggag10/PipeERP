...
class PrintSettingsRepository:
    KEYS = {
        ...
        "phones": "print_phones",
        "footer": "print_footer",
        "beneficiary_name": "print_beneficiary_name",
    ...
    DEFAULTS = {
        ...
        "beneficiary_name": "ثري إيه بايب - 3A Pipes",
    ...
    def save_settings(
        ...
        *,
        ...
        if not company_name:
            raise ValueError("اسم المصنع مطلوب في رأس الفاتورة")
        ...
        current = self._stored_asset_values()
        if logo_source:
            current["logo_path"] = self._copy_asset(logo_source, "logo")
        if qr_source:
            current["qr_path"] = self._copy_asset(qr_source, "instapay_qr")
