...
type Option = { id: string; code: string; name_ar: string; standard_weight_kg: string; unit_symbol: string };
type Options = { customers: Option[]; warehouses: Option[]; products: Option[] };
...
export function SalesPage() {
  ...
  const [options, setOptions] = useState<Options>({ customers: [], warehouses: [], products: [] });
  ...
  const visibleOrders = orders.filter((order) => order.billing_method === (tab === "weight" ? "weight" : "piece"));
  const stats = useMemo(() => ({
  ...
  async function submitPiece(event: FormEvent) {
    ...
    try {
      const created = await api<SalesOrder>("/sales/orders", { method: "POST", body: JSON.stringify({ customer_id: customerId, warehouse_id: warehouseId, notes, lines: pieceLines.map(({ product_id, quantity, unit, unit_price, notes: lineNotes }) => ({ product_id, quantity, unit, unit_price, notes: lineNotes })) }) });
  ...
  async function submitWeight(event: FormEvent) {
    ...
    try {
      const created = await api<SalesOrder>("/sales/weight-orders", { method: "POST", body: JSON.stringify({
        ...
        tax_amount: tax || "0", notes, lines: weightLines.map(({ product_id, quantity, unit, actual_weight_kg, price_per_kg, notes: lineNotes }) => ({ product_id, quantity, unit, actual_weight_kg: actual_weight_kg || null, price_per_kg: price_per_kg || null, notes: lineNotes })),
      }) });
  ...
  async function deliver() {
    ...
    try {
      const result = await api<SalesOrder>(`/sales/orders/${selected.id}/delivery`, { method: "POST", headers: { "Idempotency-Key": `sales-${crypto.randomUUID()}` }, body: JSON.stringify({ version: selected.version }) });
  ...
  async function reverseDelivery(event: FormEvent) {
    ...
    try {
      await api(`/sales/deliveries/${selected.delivery.id}/reversal`, { method: "POST", headers: { "Idempotency-Key": `sales-reversal-${crypto.randomUUID()}` }, body: JSON.stringify({ reason: reversalReason }) });
    ...
    {tab !== "quotation" ? <section className={`master-layout sales-layout ${canManageCurrent ? "" : "master-layout--single"}`}>
      ...
      {canManageCurrent && setupReady ? <article className="panel master-form-card sales-create-card"><header className="panel__head"><div><h3>{tab === "weight" ? "كارتة وزن جديدة" : "أمر بيع جديد"}</h3><p>المستند يبقى مسودة حتى الاعتماد والتسليم</p></div>{tab === "weight" ? <Scale size={20}/> : <Plus size={20}/>}</header>
        <form className="compact-form sales-form" onSubmit={tab === "weight" ? submitWeight : submitPiece}><div className="form-pair"><label>العميل<select value={customerId} onChange={(e) => setCustomerId(e.target.value)}>{options.customers.map((x) => <option key={x.id} value={x.id}>{x.name_ar} · {x.code}</option>)}</select></label><label>المخزن<select value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>{options.warehouses.map((x) => <option key={x.id} value={x.id}>{x.name_ar}</option>)}</select></label></div>
          ...
          {tab === "weight" ? <div className="sales-adjustments"><label>خصم<input type="number" min="0" step="0.01" value={discount} onChange={(e) => setDiscount(e.target.value)}/></label><label>نقل<input type="number" min="0" step="0.01" value={transport} onChange={(e) => setTransport(e.target.value)}/></label><label>ضريبة<input type="number" min="0" step="0.01" value={tax} onChange={(e) => setTax(e.target.value)}/></label></div> : null}<label>ملاحظات<textarea value={notes} onChange={(e) => setNotes(e.target.value)}/></label><button className="primary-button" disabled={submitting}>{tab === "weight" ? <Scale size={17}/> : <ShoppingCart size={17}/>} حفظ كمسودة</button></form></article> : null}
