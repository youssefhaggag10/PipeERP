...
const quantityFormat = new Intl.NumberFormat("ar-EG", {
...
const key = () => clientId();
const pieceLine = (productId = "", unit = ""): PieceDraft => ({
  key: key(),
...
const weightLine = (productId = ""): WeightDraft => ({
  ...pieceLine(productId),
  actual_weight_kg: "",
  price_per_kg: "",
});
const quoteLine = (productId = ""): QuoteDraft => ({
  key: key(),
  product_id: productId,
  item_name: "",
  quantity: "",
...
export function SalesPage() {
  ...
  const canWeightManage =
    user?.permissions.includes("weight_sales.manage") ?? false;
  const initialTab: SalesTab = location.pathname.includes("weight")
    ? "weight"
    : "piece";
  const createRequested =
    new URLSearchParams(location.search).get("focus") === "new";
  ...
  const [weightMode, setWeightMode] = useState<"total_card" | "per_line">(
    "total_card",
  );
  const [pricingMode, setPricingMode] = useState<"uniform" | "per_line">(
    "uniform",
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      const requests: [
        ...
        api<SalesOrder[]>("/sales/orders?limit=150"),
      ...
      setPieceLines((current) =>
        current.map((line) => ({
          ...line,
          product_id: line.product_id || firstProduct,
          unit: line.unit || defaultUnit,
        })),
      );
      setWeightLines((current) =>
        current.map((line) => ({
          ...line,
          product_id: line.product_id || firstProduct,
          unit: line.unit || defaultUnit,
  ...
  const visibleOrders = useMemo(
    () =>
      orders.filter(
        (order) =>
          order.billing_method === (tab === "weight" ? "weight" : "piece"),
      ),
    [orders, tab],
  );
  useEffect(() => {
    if (
      tab === "quotation" ||
      visibleOrders.some((order) => order.id === selectedId)
    )
      return;
  ...
  function productName(productId: string) {
    return options.products.find((x) => x.id === productId)?.name_ar ?? "";
  }
  function productUnit(productId: string) {
    return (
      options.products.find((x) => x.id === productId)?.unit_symbol || "قطعة"
    );
  }
  function updatePiece(lineKey: string, patch: Partial<PieceDraft>) {
    setPieceLines((rows) =>
      rows.map((x) => (x.key === lineKey ? { ...x, ...patch } : x)),
    );
  }
  function updateWeight(lineKey: string, patch: Partial<WeightDraft>) {
    setWeightLines((rows) =>
      rows.map((x) => (x.key === lineKey ? { ...x, ...patch } : x)),
    );
  }
  ...
  async function submitPiece(event: FormEvent) {
    ...
    setNotice("");
    try {
      const created = await api<SalesOrder>("/sales/orders", {
        method: "POST",
        body: JSON.stringify({
      ...
      setPieceLines([
        pieceLine(options.products[0]?.id, options.products[0]?.unit_symbol),
      ]);
      setNotes("");
      setSelectedId(created.id);
      setNotice(`تم إنشاء أمر البيع ${created.order_number} كمسودة.`);
      await load();
    } catch (reason) {
      setError(
        reason instanceof ApiError ? reason.message : "تعذر إنشاء أمر البيع",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function submitWeight(event: FormEvent) {
    ...
    setNotice("");
    try {
      const created = await api<SalesOrder>("/sales/weight-orders", {
        method: "POST",
        body: JSON.stringify({
          customer_id: customerId,
          warehouse_id: warehouseId,
          ...
          total_actual_weight_kg: netWeight || null,
          uniform_price_per_kg: uniformPrice || null,
          use_vehicle_scale: vehicleScale,
          gross_weight_kg: grossWeight || "0",
          tare_weight_kg: tareWeight || "0",
          ...
          lines: weightLines.map(
            ({
              ...
              quantity,
              unit,
              actual_weight_kg: actual_weight_kg || null,
      ...
      setSelectedId(created.id);
      setNotice(
        `تم حفظ كارتة ${created.weight_cards[0]?.card_number} كمسودة للمراجعة.`,
      );
      setWeightLines([
        {
          ...weightLine(options.products[0]?.id),
          unit: options.products[0]?.unit_symbol || "ماسورة",
        },
      ]);
      setNetWeight("");
      setNotes("");
      ...
      setError(
        reason instanceof ApiError ? reason.message : "تعذر إنشاء بيع الوزن",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function submitQuotation(event: FormEvent) {
    ...
    setNotice("");
    try {
      const created = await api<Quotation>("/sales/quotations", {
        method: "POST",
        body: JSON.stringify({
          customer_id: customerId,
          ...
          lines: quoteLines.map((line) => ({
            ...
            item_name: line.item_name || productName(line.product_id),
      ...
      setNotice(
        `تم إنشاء عرض السعر ${created.quotation_number} دون أي تأثير على المخزون أو الحسابات.`,
      );
      setQuoteLines([quoteLine(options.products[0]?.id)]);
      setValidUntil("");
      setNotes("");
      await load();
    } catch (reason) {
      setError(
        reason instanceof ApiError ? reason.message : "تعذر إنشاء عرض السعر",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function deliver() {
    ...
    setError("");
    try {
      const result = await api<SalesOrder>(
        `/sales/orders/${selected.id}/delivery`,
        {
          method: "POST",
          headers: { "Idempotency-Key": clientId("sales") },
          body: JSON.stringify({ version: selected.version }),
        },
      );
      setNotice(
        `تم التسليم وإنشاء فاتورة العميل ${result.invoice?.invoice_number}.`,
      );
      await load();
    } catch (reason) {
      setError(
