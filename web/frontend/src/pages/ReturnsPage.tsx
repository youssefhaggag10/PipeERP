...
const accountType: Record<PaymentMethod, string> = {
  cash: "cash",
  bank_transfer: "bank",
  cheque: "bank",
  wallet: "wallet",
...
export function ReturnsWorkspace({
  ...
  const [returnType, setReturnType] = useState<ReturnType>(initialType ?? "sales");
  ...
  const refundableInvoices = useMemo(
    () => invoices.filter((item) => Number(item.refundable) > 0),
    [invoices],
  );
  const compatibleAccounts = useMemo(
    () => accounts.filter((item) => item.type === accountType[paymentMethod]),
  ...
  const load = useCallback(async (type: ReturnType) => {
    ...
    setError("");
    try {
      const [invoiceRows, documentRows, refundRows, options] =
        await Promise.all([
          api<Invoice[]>(`/returns/invoices?return_type=${type}`),
          api<ReturnDocument[]>("/returns/documents?limit=250"),
          api<Refund[]>("/returns/refunds?limit=250"),
          api<{ accounts: Account[] }>("/returns/options"),
        ]);
      ...
      setInvoiceId((current) => {
        if (initialInvoiceId && invoiceRows.some((item) => item.id === initialInvoiceId))
          return initialInvoiceId;
        return invoiceRows.some((item) => item.id === current)
          ? current
          : invoiceRows[0]?.id || "";
      });
      setRefundInvoiceId((current) =>
        invoiceRows.some(
          (item) => item.id === current && Number(item.refundable) > 0,
        )
          ? current
          : invoiceRows.find((item) => Number(item.refundable) > 0)?.id || "",
      );
    } catch (cause) {
      setError(
        cause instanceof ApiError
          ? cause.message
          : "تعذر تحميل المرتجعات والاستردادات",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (initialType) setReturnType(initialType);
  }, [initialType]);

  useEffect(() => {
    void load(returnType);
  }, [load, returnType]);
  useEffect(() => {
    if (!invoiceId) {
      setLines([]);
  ...
  useEffect(() => {
    if (!compatibleAccounts.some((item) => item.id === financialAccountId))
      setFinancialAccountId(compatibleAccounts[0]?.id || "");
  ...
  async function perform(action: () => Promise<unknown>, message: string) {
    ...
    try {
      await action();
      setNotice(message);
      await load(returnType);
    } catch (cause) {
      setError(
        cause instanceof ApiError ? cause.message : "تعذر إتمام العملية",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    await perform(
      () =>
        api("/returns/documents", {
          method: "POST",
          headers: { "Idempotency-Key": clientId("return") },
          body: JSON.stringify({
            return_type: returnType,
  ...
  async function createRefund(event: FormEvent) {
    ...
    await perform(
      () =>
        api("/returns/refunds", {
          method: "POST",
          headers: { "Idempotency-Key": clientId("refund") },
          body: JSON.stringify({
            refund_type: refundType,
  ...
  async function reverse(
    kind: "documents" | "refunds",
    item: ReturnDocument | Refund,
  ) {
    const reversalReason = window.prompt("اكتب سبب العكس")?.trim();
    if (!reversalReason) return;
    await perform(
      () =>
        api(`/returns/${kind}/${item.id}/reverse`, {
          method: "POST",
          headers: { "Idempotency-Key": clientId("return-reversal") },
          body: JSON.stringify({
            version: item.version,
            reason: reversalReason,
          }),
        }),
      "تم العكس مع الاحتفاظ بالسجل التاريخي.",
    );
  }

  const typeDocuments = documents.filter(
    (item) => item.return_type === returnType,
  );
  const typeRefunds = refunds.filter(
    (item) =>
      item.refund_type ===
      (returnType === "sales" ? "customer_refund" : "supplier_refund"),
  );
  ...
  return (
    <Shell>
      <section className="page-heading returns-heading">
        <div>
          <h2>المرتجعات والاستردادات</h2>
          <p>
            مرتجع جزئي أو كامل مرتبط بفاتورة، مع تحديث ذري للمخزون والحسابات.
          </p>
        </div>
        <button
          className="secondary-button"
          onClick={() => void load(returnType)}
          disabled={loading}
        >
      ...
      <div className="sales-tabs returns-kind">
        <button
          className={returnType === "sales" ? "active" : ""}
          onClick={() => setReturnType("sales")}
        >
          <Undo2 size={17} /> مبيعات وعملاء
        </button>
        <button
          className={returnType === "purchase" ? "active" : ""}
          onClick={() => setReturnType("purchase")}
        >
          <PackageOpen size={17} /> مشتريات وموردون
        </button>
      </div>
      <div className="sales-tabs returns-tabs">
        <button
          className={tab === "documents" ? "active" : ""}
          onClick={() => setTab("documents")}
        >
          <ReceiptText size={17} /> مستندات المرتجع
        </button>
        <button
        ...
        >
          <article className="panel">
            <header className="panel__head">
              <div>
                <h3>سجل المرتجعات</h3>
                <p>المستندات المعتمدة والمعكوسة</p>
              </div>
              <span className="status-badge status-badge--active">
                {typeDocuments.length}
              </span>
            </header>
            <div className="return-document-list">
              {typeDocuments.map((item) => (
                <article
                  className={`return-document ${item.status === "reversed" ? "return-document--reversed" : ""}`}
                  key={item.id}
                >
                  <span className="return-document__icon">
                    <Undo2 size={19} />
                  </span>
                  <div>
                    <strong>
                      {item.return_number} · {item.partner_name_ar}
                    </strong>
                    <small>
                      {item.invoice_number} · {item.warehouse_name_ar} ·{" "}
                  ...
                  {canManage && item.status === "posted" ? (
                    <button
                      className="mini-action return-reverse"
                      onClick={() => void reverse("documents", item)}
                      aria-label="عكس المرتجع"
                    >
                      <RotateCcw size={15} />
                    </button>
                  ) : null}
          ...
          {canManage ? (
            <article className="panel master-form-card">
              <header className="panel__head">
                <div>
                  <h3>مرتجع {returnType === "sales" ? "مبيعات" : "مشتريات"}</h3>
                  <p>لا يمكن تجاوز المتبقي من الفاتورة</p>
                </div>
                <Undo2 size={20} />
              </header>
                  ...
                  >
                    {invoices.map((item) => (
                      <option key={item.id} value={item.id}>
                        {item.invoice_number} · {item.partner_name_ar}
                      </option>
                    ))}
                  </select>
                </label>
                {selectedInvoice ? (
                  <div className="return-invoice-summary">
                    <span>
                      ...
                      <b>
                        {currency.format(
                          Number(selectedInvoice.original_total),
                        )}
                      </b>
                    </span>
                    <span>
                      ...
                      <b>
                        {currency.format(
                          Number(selectedInvoice.returned_total),
                        )}
                      </b>
                    </span>
                    <span>
                      الصافي{" "}
                      <b>
                        {currency.format(Number(selectedInvoice.net_total))}
                      </b>
                    </span>
                  </div>
                ) : null}
                <div className="return-line-inputs">
                  {lines
                    .filter(
                      (line) =>
                        Number(
          ...
          <article className="panel">
            ...
            <div className="data-table-wrap">
              <table className="data-table">
                ...
                <tbody>
                  {typeRefunds.map((item) => (
                    <tr key={item.id}>
                      <td dir="ltr">
                        <strong>{item.refund_number}</strong>
                        <small>
                      ...
                      <td>
                        {item.invoice_number}
                        <small>{item.partner_name_ar}</small>
                      </td>
                      <td>
                        {item.financial_account_name_ar}
                        <small>{methodLabel[item.payment_method]}</small>
                      </td>
                      <td>{currency.format(Number(item.amount))}</td>
                      <td>{item.status === "posted" ? "معتمد" : "معكوس"}</td>
                      <td>
                        {canManage && item.status === "posted" ? (
                          <button
                            className="mini-action"
                            onClick={() => void reverse("refunds", item)}
                          >
                            <RotateCcw size={14} />
                          </button>
                        ) : null}
            ...
            <article className="panel master-form-card">
              <header className="panel__head">
                <div>
                  ...
                  <p>لا يمكن تجاوز الرصيد القابل للاسترداد</p>
                </div>
              ...
              <form className="compact-form" onSubmit={createRefund}>
                <label>
                  ...
                  <select
                    ...
                    required
                  >
                    {refundableInvoices.map((item) => (
                      <option key={item.id} value={item.id}>
                        {item.invoice_number} · متاح{" "}
                        {currency.format(Number(item.refundable))}
                      </option>
                    ))}
                  </select>
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    min="0.01"
                    max={
                      refundableInvoices.find(
                        (item) => item.id === refundInvoiceId,
                      )?.refundable
                    }
                    step="0.01"
                    value={refundAmount}
