        ...
        {setupReady ? <form className="compact-form purchase-form" onSubmit={submitOrder}>
          ...
          <div className="purchase-draft-lines">{draftLines.map((line, index) => {
            ...
            return <div className="purchase-draft-line" key={line.key}>
              ...
              <div className="form-pair"><label>إجمالي الكمية<input type="number" min="0.000001" step="0.000001" value={line.quantity} onChange={(event) => updateQuantity(line, event.target.value)} required /></label><label>الوحدة<input value={product?.unit_symbol ?? ""} disabled /></label></div>
              ...
              <div className="form-pair"><label>فقد الشراء<input type="number" min="0" max={line.quantity || undefined} step="0.000001" value={line.purchase_loss} onChange={(event) => updateDraftLine(line.key, { purchase_loss: event.target.value })} required /></label><label>صافي المخزن<input value={number.format(net)} disabled /></label></div>
